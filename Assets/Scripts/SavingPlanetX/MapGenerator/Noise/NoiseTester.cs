...
public class NoiseTester : MonoBehaviour
{
    ...
    public void DisplayNoise(Noise noise, GameObject plane, MapData mapData, float blackValue = 0f, float whiteValue = 1f)
    {
        int texRes = 32;
        plane.transform.localScale = new Vector3(mapData.MapWidthAbsolute / 10, 1, mapData.MapHeightAbsolute / 10);
        ...
        for(int y = 0; y < tex.height; y++)
        {
            for(int x = 0; x < tex.width; x++)
            {
                ...
                float value = (noise.GetValue(noiseX, noiseY, mapData));
